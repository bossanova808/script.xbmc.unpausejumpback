    ...
    def onPlayBackPaused(self):
        ...
        if self.isPlayingVideo():
            ...
            if isExcluded(_filename):
                ...
                log(
                    'Playback paused, setting up alarm clock - getTime()=%f getTotalTime()=%f g_jumpBackSecsAfterFwdPause=%d percentage=%f' % (
                    self.getTime(), self.getTotalTime(), g_jumpBackSecsAfterFwdPause, percentage))
                xbmc.executebuiltin('AlarmClock(JumpbackPaused, PlayerControl(SeekPercentage(%f)), 0:%d, silent)' % (
                percentage, g_waitForJumpback))
